...
__points.forEach((p) => {
    ...
    const marker = L.marker([p.lat, p.lng], { icon: makePinIcon(p) }).bindPopup(buildPopup(p), isMobile ? { maxWidth: window.innerWidth - 24 } : { minWidth: 370, maxWidth: 420 });
    marker._p = p;
...
document.querySelectorAll('input[name="overlay"]').forEach((el) => {
    el.addEventListener("change", () => {
        const val = document.querySelector('input[name="overlay"]:checked').value;
        if (val === "zipcodes") {
